...
#path_MLR2016 <- "./Working layers/P5P6_MLR_2016.shp" #clipped to P5P6
path_MLR2016 <- "./Working layers/0. Layers used/MLR2017.shp" #full length
#path_MLR2019 <- "./Working layers/P5P6_FutureMLR.shp"
...
path_poleCon <- "./Results/Flight paths/FlightPaths_2019_Valid_Poles continuity_20220328.shp"
...
path_flight19_s2n <- "./Results/Flight paths/FlightPaths_2019_Valid_raster_s2n.tif"
path_flight19_n2s <- "./Results/Flight paths/FlightPaths_2019_Valid_raster_n2s.tif"
...
load("Best poles_12m.R")
load("Best flight_12m.R")
load("Best scores_12m.R")
load("Cont Accumulation Curve_12m.R")
...
basemap16_gray +
  ...
  tm_shape(mlr_2016) +
    tm_borders(col = 'black') +
  tm_shape(flight16) +
    tm_lines(col = "goldenrod1", alpha = 0.35, lwd = 1.5) +
  #tm_shape(flight16_Q3) +
  #  tm_lines(col = "orangered2", alpha = 0.2) +
  tm_shape(flight16_whiskUp) +
    tm_lines(col = "firebrick3", alpha = 0.5, lwd = 1.5)
...
#print plots
#h826*1240
png("Figure 6_map+box.png", height =  4130 , width =  6200, units = "px", res = 600)
...
print(base16_legend, vp = viewport(x = 0.63, y = 0.7,
...
ggplot(ContAccum, aes(x = Generation, y = MaxConnectivity)) + 
  geom_rect(xmin=8, xmax=17, ymin=0, ymax=Inf, fill = "gray95") +
  geom_rect(xmin=28, xmax=43, ymin=0, ymax=Inf, fill = "gray95") +
  geom_step(size = 1) +
  theme(panel.grid = element_blank(),
        panel.background = element_blank(),
        panel.border = element_rect(colour = "black", fill = NA, size = 1),
  ...
  annotate(geom = "text", x = 11, y = 85, 
           label = "n=9", hjust = "left", size = 7) +
  annotate(geom = "text", x = 32.5, y = 85, 
           label = "n=15", hjust = "left", size = 7) +
...
test <- merge(UniBi, best_score %>% as.data.frame(), by = "ID", all = T) %>%
...
tm_shape(best_poles2) +
  ...
  tm_text("ID", size = 1.3, just = "right", 
...
## Calculate amount of pixel with glide path i.e., Coverage

```{r Rasterize for coverage, include = F}

flight22_buffer <- st_buffer(flight22, 2, endCapStyle = 'FLAT')

#rasterize each direction first
mlrSpat <- vect(mlr_2019)
raster_template = rast(mlrSpat, resolution = 0.5)

flight22_raster = rasterize(vect(flight22_buffer), raster_template, sum = TRUE, progress = 'text')

plot(flight22_raster)
