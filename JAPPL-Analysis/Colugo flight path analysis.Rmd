...
path_trees2016 <- "./Working layers/Trees_50mMLR_2016_P5P6MLR.shp"
...
#variable control panel
path_dem2016 <- "./Working layers/DEM_2016.tif"
path_MLRbuffer <- "./Working layers/P5P6_FutureMLR_50mBuffer.shp"

#load data
dem16<- raster(path_dem2016) %>%
  projectRaster(crs=crs(flight16))
  
mlr_buffer <- st_read(path_MLRbuffer)

#crop raster to extent to reduce analysis time
dem16_Crop <- crop(x = dem16, y = mlr_buffer)

#### Measure raster values ####
#extract max raster value along each flight path
flightSamp <- flight16 %>%
  mutate(maxElev = raster::extract(x = dem16_Crop, y = .,
                                   fun = max, progress = 'text'))
...
#variable control panel
path_trees2019 <- "./Working layers/TreesPoles_Remain2019_P5P6MLR_20210604.shp"
genus_exclude <- 'Elaeis|Oncosperma|Strychnos|Ziziphus|Clerodendrum|Dillenia|Calliandra'
...
#### Save shapefile ####
st_write(flight19, "./Results/Flight paths/FlightPaths_2019_20220324.shp")
...
#### Measure raster values ####
flightSamp <- flight19 %>%
  mutate(maxElev = raster::extract(x = dem19_Crop, y = flight19,
                                   fun = max, progress = 'text'))
...
#### Clear tree height WITHOUT hoarding and poles
...
#### Clipping flight paths with hoarding
...
#before invalide flight paths were removed
...
st_write(flight19_valid_H2, "./Results/Flight paths/FlightPaths_2019_Valid_H2_20220328.shp", crs=3414)

#st_write(poles_valid_H2, "./Results/Flight paths/FlightPaths_2019_Valid_Poles continuity_20220328.shp", crs=3414)
...
#rasterization
flight19_n2s_raster = rasterize(flight19_valid_n2s_buffer, raster_template, field = 'Diff_maxH2', fun = 'count', progress = 'text')
flight19_s2n_raster = rasterize(flight19_valid_s2n_buffer, raster_template, field = 'Diff_maxH2', fun = 'count', progress = 'text')

flight19_raster_total = rasterize(flight19_valid_H2_buffer, raster_template, field = 'Diff_maxH2', fun = 'count', progress = 'text')
...
path_flight19 <- "./Results/Flight paths/FlightPaths_2019_Valid_H2_20220328.shp"
path_trees2019 <- "./Working layers/TreesPoles_Remain2019_P5P6MLR_20210604.shp"
genus_exclude <- 'Elaeis|Oncosperma|Strychnos|Ziziphus|Clerodendrum|Dillenia|Calliandra'
...
flightSamp <- flight19_poles_full %>%
  mutate(maxElev = raster::extract(x = dem19_Crop, y = flight19_poles_full,
                                   fun = max, progress = 'text'))
...
##run through code chunk 8 and 9 accordingly and save out "poles_valid_H2"
...
#filter away trees with no connected paths
connected16 <- trees16 %>%
  filter(TreeID %in% flight16$TreID_T | TreeID %in% flight16$TrID_Fr) %>%
  left_join(TreeFrm16, by = c("TreeID" = "TrID_Fr"), keep = F) %>%
  left_join(TreeTo16, by = c("TreeID" = "TreID_T"), keep = F) %>%
  replace(is.na(.), 0) %>%
  mutate(PathTotal = PathFrm + PathTo) %>%
  mutate(Year = "2016") %>%
  mutate(Type = 'Tree') %>%
  relocate(Year, TreeID, Type, Species, Girth_M, Height_M, Elev, MLRLoc, PathFrm, PathTo, PathTotal) %>%
  dplyr::select(-c("Lat", "Long", "GlideRadii", "FinalStat", "Height_GR"))
...
connected19 <- trees19 %>%
  ...
  left_join(TreeFrm19, by = c("ID" = "ID_Frm"), keep = F) %>%
  left_join(TreeTo19, by = c("ID" = "ID_To"), keep = F) %>%
  replace(is.na(.), 0) %>%
  mutate(PathTotal = PathFrm + PathTo) %>%
  mutate(Year = "2019") %>%
  relocate(Year, ID, Type, Species, Girth_M, Height_M, Elev, MLR_Loc, PathFrm, PathTo, PathTotal) %>%
  dplyr::select(-c("Lat", "Long", "GlideRadii", "Height_GR")) %>%
  dplyr::rename(TreeID = ID) %>%
  dplyr::rename(MLRLoc = MLR_Loc)

#save out shapefile for QGIS
st_write(connected16, "./Results/Nodes/Trees_2016_Valid.shp", crs=3414)
st_write(connected19, "./Results/Nodes/Trees_2019_Valid.shp", crs=3414)

#drop geometry to only use data frame for exploratory analysis
#results not included in analysis
connected16_df <- connected16 %>% 
  st_drop_geometry()
connected19_df <- connected19 %>% 
  st_drop_geometry()

connected <- bind_rows(connected16_df, connected19_df) %>%
  gather(Directional, PathNum, PathFrm:PathTotal)

#list of plants used in valid flight paths
plants <- data.frame(matrix(ncol = 1, nrow = length(unique(connected$Species)))) %>%
  mutate(Species = unique(connected$Species)) %>%
  dplyr::select(Species) %>%
  arrange(Species) %>%
  mutate(Genus = gsub( " .*$", "", Species))

write.csv(plants, "./Results/Nodes/Plant Species.csv")
  
#density plot with rug
ggplot(connected, aes(x=PathNum, colour = Year)) +
  geom_density() +
  facet_wrap(vars(Directional)) +
  geom_rug(sides="b") +  #rug draws ticks for each value
  theme_minimal() + 
  scale_x_continuous(limits = c(0, 100))

#boxplot with variale width
my_xlab <- paste(levels(connected$Year),"\n(N=",table(connected$Year),")",sep="")

ggplot(connected, aes(x=Year, y=PathNum, fill=Year)) +
...
model <- glm(PathNum~Year*Directional, data = connected, family = poisson())
sum(residuals(model)^2)/(df.residual(model))
summary(model)
anova(model)

model.nb <- glm.nb(PathNum~Year*Directional, data = connected)
summary(model.nb)
anova(model.nb)

connectedFrm <- connected %>% filter(Directional %in% 'PathFrm')

frm.nb <- glm.nb(PathNum~Year, data = connectedFrm)
summary(frm.nb)
anova(frm.nb)
...
path_flight19_raster <- "./Results/Flight paths/FlightPaths_2019_Valid_raster.tif"
path_flight2022 <- "./Results/Flight paths/FlightPaths_2022_Valid.shp"
path_MLRbuffer <- "./Working layers/P5P6_FutureMLR_50mBuffer.shp"

#load data
bound <- st_read(path_plotArea)
lr_buffer <- st_read(path_MLRbuffer)
...
flight22_raster = rasterize(flight22_buffer, raster_template, field = 'Diff_H', fun = 'count', progress = 'text')
